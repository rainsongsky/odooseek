...
import { O2mKanbanView } from './o2m_kanban'
import { normalizeO2mValue, O2mCellDisplay, O2mCellEdit } from './shared.tsx'

export interface One2ManyWidgetHandle {
  validate: () => { valid: boolean; errors: string[] }
}

export const One2ManyWidget = forwardRef<One2ManyWidgetHandle, FieldWidgetProps>(
  function One2ManyWidget({ field, value, onChange, readOnly, meta, widgetRef }, _fwdRef) {
    ...
    const rawMode = field.mode ?? 'list'
    const mode = rawMode.includes(',')
      ? (rawMode
          .split(',')
          .map((m) => m.trim())
          .find((m) => m === 'kanban' || m === 'tree' || m === 'list') ?? 'list')
      : rawMode
    const isKanbanMode = mode === 'kanban' && !!field.subViews?.kanban
    ...
    const fieldNames = useMemo(() => {
      if (isKanbanMode) {
        const kanbanFields = (field.subViews?.kanban as { fields?: string[] } | undefined)?.fields
        return kanbanFields && kanbanFields.length > 0 ? kanbanFields : ['name', 'display_name']
      }
      return columns.map((c) => c.name)
    }, [columns, isKanbanMode, field.subViews])
    ...
    useImperativeHandle(
      widgetRef as React.Ref<One2ManyWidgetHandle> | undefined,
      () => ({
        validate: () => {
          const errors: string[] = []
          for (const col of columns) {
            ...
            for (const record of displayRecords) {
              if (isFieldValueEmpty(record[col.name], fieldType)) {
                const label = col.string || autoFields?.[col.name]?.string || col.name
                const displayName = (record.display_name || record.name || record.id) as string
                errors.push(`${label} (${displayName})`)
              }
            }
          }
      ...
      [columns, autoFields, displayRecords],
    )
    ...
    if (isKanbanMode) {
      return (
        <O2mKanbanView
          relation={relation}
          ids={ids}
          template={(field.subViews?.kanban as { template?: string } | undefined)?.template ?? ''}
          templateFields={
            (field.subViews?.kanban as { fields?: string[] } | undefined)?.fields ?? []
          }
          fields={autoFields ?? {}}
          readOnly={readOnly}
          canCreate={subViewList?.create !== false}
          canDelete={subViewList?.delete !== false}
          onAdd={handleAddRow}
              ...
              {displayRecords.map((record) => {
                ...
                if (useSubForm && subViewForm) {
                  ...
                  return (
                    <tr
                      key={String(rowId)}
                      className={`border-b border-border-subtle ${decoClass}`}
                    >
                      <td
