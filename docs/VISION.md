...
> Odoo 的声明式元数据架构是它最宝贵的资产。我们不需要重写 CRM、会计引擎、工作流——我们需要的是：保留这一切，但给用户一个世界级的前端体验。

答案就是 **oweb** — 一个基于 React + TypeScript + Bun + TanStack/Router + Rust BFF 的 Odoo 前端项目，它：

- **保留** Odoo 的元数据驱动内核（`ir.*` 表、XML 视图声明、声明式权限）
- **保留** Odoo 的业务逻辑（Python ORM、工作流引擎、报表、自动化）
- **替换** Odoo 的前端渲染层（OWL/QWeb → React SPA）
- **替换** Odoo 的通信层（Python WSGI 直连 → Rust BFF 网关）
...
Odoo 本质上是一个 **元数据解释器**。它的核心哲学是：

> **数据定义程序，而非程序定义数据。** — 模型、视图、菜单、权限、自动化流程，全部以声明式元数据的形式存储在数据库中，而非硬编码在源代码中。

具体表现：

| Odoo 元数据表 | 存储内容 | 运行时行为 |
|---------------|----------|------------|
| `ir.model` / `ir.model.fields` | 模型定义与字段属性 | 向数据库插入一条记录，即可动态新增字段——无需修改代码 |
| `ir.ui.view` | 表单、列表、看板的 XML 结构 | 前端请求时从数据库读取 XML，解析、继承、合并后渲染 |
| `ir.ui.menu` / `ir.actions.*` | 菜单项与动作定义 | 导航结构由数据库记录驱动，前端动态构建菜单 |
| `ir.rule` | 记录级权限规则 | `[('company_id','=', user.company_id)]` 声明式过滤，无需 SQL |
| `ir.cron` / `base.automation` | 定时任务与自动化 | 声明触发条件与执行动作，无循环或条件分支 |

OdooSeek **完整保留这套元数据驱动架构**：

- ✅ **模型定义** — 仍由 Odoo ORM 管理，oweb 通过 JSON-RPC 查询 `ir.model.fields` 获取字段元数据
- ✅ **视图定义** — 仍存储在 `ir.ui.view` 表中，oweb 动态获取并解析 XML 视图结构
- ✅ **菜单与权限** — 仍由 Odoo 内置模块管理，oweb 通过 API 拉取后渲染
- ✅ **业务逻辑** — 全部保留在 Odoo 服务端，不迁移、不重写

**我们改变的，只是前端渲染层和通信中间件**：

```
Odoo 原生架构                          OdooSeek 架构
────────────                           ────────────
Python OWL + QWeb                      React + TypeScript
  (服务端渲染, 模板引擎)                  (客户端 SPA, 动态组件)
  ↓                                     ↓
  ↑ 直连 Odoo Server                    odoo-web-server (Rust BFF)
  ↑                                     ↓
  ↑ Python WSGI                         Odoo 19 CE (JSON-RPC)
                                         (元数据/业务逻辑保持不变)
```

> **"我们不是在逃离 Odoo，而是在拓宽它的边界。"** — OdooSeek 保留了 Odoo 元数据驱动的设计灵魂，用现代 Web 技术赋予它一个全新的前端表达。
